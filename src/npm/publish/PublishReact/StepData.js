  ...
  steps1: {
    stepList: [
      ...
      '创建测试组件库例子的文件夹',
      {
        ...
        subTitle:
          '1. 通过bable-loader转译.js和.jsx文件 2. 通过css-loader和style-loader解决样式依赖 3. 使用html-webpack-plugin导入scripts 4. 本地服务端口号设置为3001'
      },
      {
      ...
      },
      {
        ...
        subTitle:
          '使用peerDependency让npm在发布的程序包中不包括React，这减小了程序包的大小，并避免了在用户目标项目中具有多个版本的React的灾难。'
      },
      {
        title: 'Step6.3 修改package.json',
        subTitle:
          '添加一个transpile脚本，使用该脚本使Babel转换组件源文件，并将所有assets（例如.css文件）复制到名为lib的目标文件夹中'
      },
      {
        title: 'Step6.4 修改package.json',
        subTitle:
          '添加一个prepublishOnly脚本，便于在每次使用npm publish发布时自动执行transpile脚本'
      },
      '发布到npm'
    ],
    contentList: [
